...
export default function MedicineScanner({ onIngredientFound, onClose }: MedicineScannerProps) {
  ...
  const processImage = async (src: string | null) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('dashboard.failed'));
      }
      ...
      if (data.ingredient) {
        ...
        setError(t('dashboard.scanner.error_identify'));
      }
    ...
    <div className="fixed inset-0 z-[100] flex flex-col bg-[#101d22] text-slate-100 font-display animate-fade-in overflow-hidden">
      ...
      <header className="flex items-center justify-between border-b border-[#18262b]/50 bg-[#101d22]/90 backdrop-blur-md px-6 py-4 absolute w-full z-50">
        <div className="flex items-center gap-3 text-white">
          ...
          <h2 className="text-white text-lg font-bold leading-tight font-display tracking-tight">{t('dashboard.scanner.title')}</h2>
        </div>
            ...
            <div className="flex justify-between items-start pt-[60px]">
              <div className="flex flex-col gap-1">
                <div className="flex items-center gap-2 text-[#13b6ec]/70 mb-1">
                  <span className="text-xs font-mono tracking-wider uppercase">{t('dashboard.scanner.patient_id')}</span>
                </div>

                <div className="h-px w-32 bg-[#13b6ec]/30"></div>
                <div className="text-white/80 font-mono text-xs mt-1">
                  {t('dashboard.scanner.sys_active')} <span className="text-white/40">|</span> O2: 98%
                </div>

              </div>
              <div className="flex flex-col items-end gap-1">
                <div className="flex items-center gap-2 text-[#13b6ec]/70 mb-1">
                  <span className="text-xs font-mono tracking-wider uppercase">{t('dashboard.scanner.mode_diagnostic')}</span>
                </div>
            ...
            <div className="flex flex-col items-center gap-6 w-full max-w-2xl mx-auto pb-[100px]">
              <div className="flex flex-col items-center gap-2">
                <div className="flex items-center gap-2 px-4 py-1.5 bg-[#101d22]/80 backdrop-blur rounded-full border border-[#13b6ec]/20">
                  ...
                  <span className="text-[#13b6ec] font-mono text-sm tracking-widest uppercase">{isProcessing ? t('dashboard.scanner.analyzing') : t('dashboard.scanner.scanning')}</span>
                </div>
                <p className="text-white/50 text-xs font-mono">{isProcessing ? t('dashboard.scanner.extracting_api') : t('dashboard.scanner.keep_subject')}</p>
